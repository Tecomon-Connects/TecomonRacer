export function item(context_object) {
    ...
    function delta_move(nx, ny) {
        let collideX = x, collideY = y;

        collideX += nx;
        collideY += ny;

        if (!context.isPointInPath(context_object.pathIn, collideX, collideY) && context.isPointInPath(context_object.pathOut, collideX, collideY)) {
            x += nx;
            y += ny;
            needupdate = true;
        }
    ...
    return {
        move, rotate, grab, rotateRadians,
        isTouched, touchEnd, getMatrix, draw, delta_move,
        append, setScale, obj_infos, get x() { return x }, get y() { return y }
    };
}
...
export function imgCar(context_object, img, x, y) {
    ...
    return {
        draw, move, delta_move, rotate, rotateRadians, reset, setScale, get x() { return x }, get y() { return y }
    };
}
...
function tri_path() {   //green touch triangle
...
export function getCanvas(id, width, height) {
    ...
    context_object.pre_draw = function (runden1, runden2, color1, color2) {
        context.clearRect(0, 0, context.canvas.width, context.canvas.height);
        context.setTransform(matrix);

        draw_canvas(runden1, runden2, color1, color2);
    }

    function draw_canvas(runden1, runden2, color1, color2) {
        ...
        context.fillText(`Runden: ${runden1}`, canvas.width - 200*scale, 0 + 40*scale);
        context.fillStyle = color2;
        if (runden2 == -1) runden2 = 0;
        context.fillText(`Runden: ${runden2}`, canvas.width - 200*scale, 0 + 90*scale);
        context.restore();
        
        //Strecke  
        pathOut = new Path2D();
        pathOut.rect(canvas.width/2 - canvas.width/3, canvas.height/2 - canvas.height/3, canvas.width/1.5, canvas.height/1.5);
        context.stroke(pathOut);
 
        pathIn = new Path2D();
        pathIn.rect(canvas.width/2 - canvas.width/4 + 50, canvas.height/2 - canvas.height/4 + 50, canvas.width/2 - 100, canvas.height/2 - 100);
        ...
        goal = new Path2D();
        goal.rect(canvas.width/2 - canvas.width/3 + canvas.width/3, canvas.height/2 - canvas.height/3, 20, canvas.height/12 + 50);
        context.fill(goal);
        context.restore();
    }
...
export function winGame(context, spieler, color, fontSize, scale) {
    context.fillStyle = "#eee";
    context.roundRect(context.canvas.width/2 - 200*scale, context.canvas.height/2 - 100*scale, 400*scale, 200*scale, 20);
    context.fill();
    context.translate(context.canvas.width/2 - 200*scale, context.canvas.height/2 - 100*scale);
